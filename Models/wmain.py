...
def generator(b_s, phase_gen='train'):
    if phase_gen == 'train':
        images =[imgs_path + f for f in os.listdir(imgs_path) if "train" in f and f.endswith('.jpg')]
        maps = [maps_train_path + f for f in os.listdir(maps_train_path) if f.endswith('.mat')]
    elif phase_gen == 'val':
        images = [imgs_path + f for f in os.listdir(imgs_path) if "val" in f and f.endswith('.jpg')]
        maps = [maps_val_path + f for f in os.listdir(maps_val_path) if f.endswith('.mat')]
    else:
    ...
    if phase == 'train':
        ...
        model.fit_generator(generator(b_s=b_s), nb_imgs_train, nb_epoch=nb_epoch,
                            ...
                            callbacks=[EarlyStopping(patience=5),
                                       ModelCheckpoint('wSMweights/weights.mlnet.{epoch:02d}-{val_loss:.4f}.pkl', save_best_only=True)])

    elif phase == "test":
        # path of output folder
        output_folder = 'wSALICON_preds_mats/'
        ...
        file_names=[f for f in os.listdir(imgs_test_path) if "val" in f and f.endswith('.jpg')] ##Modificat per mi
        file_names.sort()
        nb_imgs_test = len(file_names)

        print("Load weights ML-Net")
        model.load_weights('wSMweights/weights.mlnet.19-0.0326.pkl')
